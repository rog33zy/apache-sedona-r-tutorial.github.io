...
And is partitioned equally for optimised wide operations, especially joins. Wide operations are those that require data shuffling or exchange between multiple executors such as aggregations and joins. Meanwhile, narrow transformations do not require any exchange of data. Examples of narrow transformations include select and filter.
...
Upon registration, we can now conduct a **spatial join**, asking **Apache Sedona** to find neighbourhoods that contain specific coordinates using the **ST_Contains** function. You can find documentation on all available **Apache Sedona** vector functions [here](https://sedona.apache.org/docs/latest/sql-functions/).
