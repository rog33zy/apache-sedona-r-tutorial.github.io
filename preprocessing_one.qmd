...
We use `sparklyr` to interface with [Apache Spark](https://aws.amazon.com/what-is/apache-spark/) in R, allowing us to work efficiently with large datasets using distributed computing. Spark operates on a cluster-based architecture, where a **driver** program coordinates tasks and **executors** perform computations across multiple nodes in parallel. However, when clusters are unavailable, Spark can also run in **local mode**, using a single machine while still leveraging parallelism to speed up computations. This makes it accessible for development and smaller-scale analyses. The `dplyr` package provides powerful data manipulation functions that integrate seamlessly with Spark, making it easier to transform and summarise data. Finally, we load `arrow`, which enhances Spark’s [performance](https://arrow.apache.org/blog/2019/01/25/r-spark-improvements/) when copying, collecting, and transforming data, thereby improving the overall efficiency of our analysis.
